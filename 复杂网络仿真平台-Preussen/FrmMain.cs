...
namespace CNSP
{
    ...
    {
        int[,] Map;           //全局变量，节点坐标映射
        public int PicWidth;         //全局变量，图像宽度
        public int PicHeight;        //全局变量，图像高度
        ...
        private void OpenMI_Click(object sender, EventArgs e)
        {
            ...
            this.DiaOpen.Filter = "All files (*.*)|*.*|Standard list files (*.sst)|*.sst";
        ...
        private Boolean SaveNetwork()
        {
            ...
            TSSL1.Text = "正在保存";
            //初始化对话框，文件类型，过滤器，初始路径等设置
            this.DiaSave.Filter = "Standard list files (*.sst)|*.sst";
       ...
       private void TabMain_DoubleClick(object sender, EventArgs e)
       {
        ...
        //通信行为仿真菜单响应函数
       private void SimulateMI_Click(object sender, EventArgs e)
       {
           FrmComBehavior frmSimulate = null;
           if (ComplexNet == null)
           {
               MessageBox.Show("请先读取或生成一个网络！", "警告", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
               return;
           ...
           if (frmSimulate == null)
           {
               frmSimulate = new FrmComBehavior(this);
           }
